 ...
 * @param {args=} args - args to be passed in as arguments to command being run as a new script.
 **/
export async function getNsDataThroughFile(ns, commandOrFunc, fName, args = [], verbose = false, maxRetries = 5, retryDelayMs = 50) {
    checkNsInstance(ns, '"getNsDataThroughFile"');
    if (!verbose) disableLogs(ns, ['read']);
    const commandHash = typeof commandOrFunc === 'string' ? hashCode(commandOrFunc) : hashCode(commandOrFunc.toString());
    ...
    let commandToFile;
    if (typeof commandOrFunc === 'string') {
        commandToFile = `let r;try{r=JSON.stringify(\n` +
            `    ${commandOrFunc}\n` +
            `);}catch(e){r="ERROR: "+(typeof e=='string'?e:e.message||JSON.stringify(e));}\n` +
            `const f="${fName}"; if(ns.read(f)!==r) await ns.write(f,r,'w')`;
    } else {
        const strFuncArgs = JSON.stringify(args);
        commandToFile = `let r;try{r=JSON.stringify(\n` +
            `    (${commandOrFunc})(${strFuncArgs})\n` +
            `);}catch(e){r="ERROR: "+(typeof e=='string'?e:e.message||JSON.stringify(e));}\n` +
            `const f="${fName}"; if(ns.read(f)!==r) await ns.write(f,r,'w')`;
    }
    // Run the command with auto-retries if it fails
    const pid = await runCommand_Custom(ns, ns.run, commandToFile, fNameCommand, args, verbose, maxRetries, retryDelayMs);
    ...
    const fnIsAlive = (ignored_pid) => ns.read(fName) ===
}
